...
class BERT(nn.Module):
    ''' Transformer_encoder 그냥 쌓아놓기만 하면 댐'''
    ''' input representation 설정'''
    ''' input emb + pos emb'''
    ''' key, query, value emb'''
    '''EncoderLayer 반복'''
    def __init__(self, config):
...
def extract_answer_span(inputs):
    bert_answer = []
    for bert_output in inputs:
        sep_index = 0
        for i, token in enumerate(bert_output):
            if token == sep_token_id:
                sep_index = i
        bert_answer.append(torch.tensor(bert_output[sep_index + 1:]))
    
    return torch.nn.utils.rnn.pad_sequence(bert_answer, batch_first= True, padding_value= 0)


def SQuAD_train_epoch(config, model, criterion, optimizer, data_loader):
    losses = []
    model.train()

    for i, value in enumerate(data_loader):
        sep_token_indices, answers, bert_inputs = map(lambda x: x.to(config.device), value)
        bert_outputs, token_emb = model(bert_inputs, sep_token_indices)

        #(batch_size, n_seq, d_hidn)
        answer_span = extract_answer_span(bert_outputs)
        answers_start, answers_end = list(zip(*answers))

        #(batch_size, n_seq)
        scores_start = torch.matmul(answer_span, token_emb(torch.stack(answers_start)).transpose(0,1))
        scores_end = torch.matmul(answer_span, token_emb(torch.stack(answers_end)).transpose(0,1))

        #(batch_size)
        argmax_start = torch.argmax(scores_start, dim =1)
        argmax_end = torch.argmax(scores_end, dim=1)

        optimizer.zero_grad()

        loss = criterion(scores_start, argmax_start) + criterion(scores_end, argmax_end)
        losses.append(loss.item())
        loss.backward()
...
def SQuAD_eval_epoch(model, config, data_loader):
    '''F1 score로 QA tasks에서 BERT 성능 평가'''
    '''training epochs 별로 성능의 변화도를 그래프로 나타내면 좋을듯'''
    '''  !! 모델 퍼포먼스를 중간에 저장하기 '''
    scores = []
    model.eval()
...
def SQuAD_task(config, vocab):
    ''' fine-tuning 데이터 로드 SQuAD '''
    ''' input = (question [sep] context), answer = S, E of answer sentene'''
    ''' 학습시 softmax를 maximize 하도록 CrossEntropyloss를 사용'''
    ''' test시 또 따로 생각'''
    losses = []
    scores = []
    ...
    train_dataset = BERTDataset(vocab, './SQuAD_train.json', 'SQuAD')
    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size = config.batch_size, shuffle = True, collate_fn = SQuAD_collate_fn)

    test_dataset = BERTDataset(vocab, './SQuAD_test.json', 'SQuAD')
    test_loader = torch.utils.data.DataLoader(test_dataset, batch_size = config.batch_size, shuffle = True, collate_fn = SQuAD_collate_fn)

    for epoch in range(config.n_epoch_SQuAD):
        loss = SQuAD_train_epoch(config, model, criterion, optimizer, train_loader)
        score = SQuAD_eval_epoch(config, model, test_loader)
...
#순서
#tokenize datasets
#[cls] sentenceA [sep] sentence B [sep] are consecutive with 50 % probability
#masking 15% <- 언제해야할까
#
#batch단위로 pretraining 시작
#masked token prediction
#   we have to remember which words are masked
#next sentence prediction
#   supervised learning using IsNext and NotNext token 이 될수 있지만
#   IsNext or NotNext token이 나올 확률을 높이는 unsupervised 방법으로 한다.
#두개 task에 대해 동시에 학습!